...
import type { Cobranca, Comissao } from '@/types';
import { CheckCircle2, MessageCircle, Plus, RefreshCw, XCircle } from 'lucide-react';

type Aba = 'pendentes' | 'pagas' | 'todas' | 'comissoes';
...
export default function FinanceiroPage() {
  ...
  async function carregar() {
    setCarregando(true);
    const [cob, com] = await Promise.all([
      supabase
        .from('cobrancas')
    ...
    setCobrancas((cob.data as Cobranca[]) ?? []);
    setComissoes((com.data as unknown as Comissao[]) ?? []);
    setCarregando(false);
  }
  ...
  // Gera mensalidades dos revendedores master (clientes ativos × valor por acesso)
  async function gerarMensalidades() {
    ...
    try {
      ...
      if (novas.length === 0) {
        toast('Nenhuma mensalidade nova (já geradas neste mês ou sem clientes ativos).');
      } else {
        const { error } = await supabase.from('cobrancas').insert(novas);
        if (error) throw new Error(error.message);
        toast(`${novas.length} mensalidade(s) de revendedor gerada(s).`);
        await carregar();
      }
    } catch (e: any) {
      toast(`Erro: ${e.message}`, 'erro');
  ...
  async function confirmarPagamento() {
    ...
    try {
      ...
      // Gera comissão para o indicador, se houver
  ...
  return (
    <div>
      <PageTitle
        title="Financeiro"
        subtitle={`A receber: ${fmtMoeda(totalPendente)} · Comissões pendentes: ${fmtMoeda(totalComissoesPendentes)}`}
        action={
          <div className="flex gap-2">
            ...
            <Btn variant="secondary" onClick={gerarMensalidades} disabled={gerando}>
              <Plus size={15} /> Mensalidades revendas
            </Btn>
          </div>
        }
      />
      ...
      {carregando ? (
        ...
        comissoes.length === 0 ? (
          <Vazio>Nenhuma comissão gerada ainda. Elas são criadas automaticamente ao registrar o pagamento de um cliente indicado.</Vazio>
        ) : (
          <Tabela>
            <thead>
              <tr>
        ...
        <Vazio>Nenhuma cobrança aqui. Use os botões acima para gerar renovações e mensalidades.</Vazio>
      ) : (
        <Tabela>
          <thead>
            <tr>
