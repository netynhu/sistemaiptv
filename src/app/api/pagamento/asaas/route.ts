...
// Gera uma cobrança PIX real no Asaas a partir de uma cobrança já existente no sistema
// (Financeiro > Receitas > "Gerar cobrança Asaas"). Guarda o id externo para o webhook
// conseguir dar baixa automática quando o cliente pagar.
export async function POST(req: NextRequest) {
  try {
    ...
    if (!pagamentos?.asaas_token) {
      return NextResponse.json({ error: 'Token do Asaas não configurado. Acesse Configurações > Pagamentos.' }, { status: 400 });
    }
