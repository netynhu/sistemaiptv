...
// Cada tela que usa Assist Plus custa `custo_por_tela` (Configurações > telas). As demais
...
export async function sincronizarDespesaAssistPlus(clienteId: string): Promise<void> {
  ...
  const telasCfg = await getSetting<{ custo_por_tela: number | string }>('telas_config');
  const custoPorTela = Number(telasCfg?.custo_por_tela ?? 1.5) || 1.5;
